...
import { fetchCrmSmsApplication } from '@/components/crm/hooks'
...
export default defineComponent({
    ...
    setup(props, { emit }) {
        ...
        const { state, fetchRefresh } = useColumnService({
            request: (base, payload) => Service.httpBaseCrmClientSmsColumn(payload),
            formState: { clientId: faseNode.value.keyId },
            immediate: true
        })

        /**添加短信应用**/
        async function fetchCreateSmsAppApplication() {
            return await fetchCrmSmsApplication({
                title: '添加短信应用',
                command: 'CREATE',
                node: { clientId: faseNode.value.keyId }
            })
        }

        return () => (
            <common-element-scrollbar class="crm-client-context-sms-wrapper">
                <crm-client-context-sms-skeleton initialize={state.initialize}>
                                            ...
                                            <div class="flex gap-x-10 p-inline-14 p-block-14 overflow-hidden">
                                                ...
                                                <div class="m-be-auto flex items-center">
                                                    <common-database-table-chunk
                                                        element="chunk"
                                                        value={item.type}
                                                        options={props.chunkState.CHUNK_CLIENT_SMS_TYPE}
                                                    ></common-database-table-chunk>
                                                </div>
                                            </div>
